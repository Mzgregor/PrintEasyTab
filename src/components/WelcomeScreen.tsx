...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onCreateTab }) => {
    const { language } = useSongStore();

    return (
        <div className="flex flex-col items-center justify-center min-h-[600px] h-full py-12 px-6 text-center animate-in fade-in duration-700">
            {/* Logo Container with Glow effect */}
            <div className="relative mb-12 group">
                <div className="absolute inset-x-0 -inset-y-4 bg-accent/20 blur-3xl rounded-full opacity-50 group-hover:opacity-100 transition-opacity duration-1000"></div>
                <img
                    src="/LOGO_1_OMT.png"
                    alt="One More Tab"
            ...
            <div className="space-y-4 mb-14 max-w-2xl">
                <h1 className="text-5xl font-black uppercase tracking-tighter text-text-primary leading-tight">
                    {language === 'fr' ? (
                        <>Prêt pour votre prochaine <span className="text-accent underline decoration-8 decoration-accent/20 underline-offset-8">création</span> ?</>
                    ) : (
                        <>Ready for your next <span className="text-accent underline decoration-8 decoration-accent/20 underline-offset-8">creation</span> ?</>
                    )}
                </h1>
            ...
            <button
                onClick={onCreateTab}
                className="group relative flex items-center gap-4 px-12 py-6 bg-accent text-white rounded-[2.5rem] font-black uppercase text-lg tracking-[0.2em] shadow-[0_20px_40px_rgba(56,189,248,0.3)] hover:shadow-[0_25px_50px_rgba(56,189,248,0.5)] hover:scale-105 hover:-translate-y-1 active:scale-95 transition-all duration-300 overflow-hidden"
            >
